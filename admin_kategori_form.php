...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name)) {
        ...
        if ($stmt->execute()) {
            header("Location: admin_kategori_list.php?status=sukses");
            exit();
        } else {
...
if ($category_id > 0) {
    $sql_fetch = "SELECT name, description FROM kategori WHERE id = ?";
    ...
    if ($result->num_rows === 1) {
        ...
        die("<h1 class='text-red-600 p-6 font-bold'>Data kategori tidak ditemukan!</h1>");
    }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title><?= $is_edit ? "Edit Kategori" : "Tambah Kategori Baru" ?> - Admin GALAK</title>
...
<body class="bg-backgroundLight dark:bg-backgroundDark text-textLight dark:text-textDark transition-colors duration-300 min-h-screen flex flex-col">
  ...
  <main class="container mx-auto px-6 py-12 flex-grow">
    <h1 class="orbitron text-4xl mb-10 text-center select-none"><?= $is_edit ? "Edit Kategori" : "Tambah Kategori Baru" ?></h1>
    <form method="post" class="glass-card p-8 rounded-2xl max-w-2xl mx-auto space-y-6" novalidate>
      ...
      <div>
        <label for="name" class="block mb-2 font-semibold text-primaryLight dark:text-primaryDark">Nama Kategori</label>
        <input type="text" id="name" name="name" required value="<?= e($name) ?>" placeholder="Contoh: Action" class="w-full p-2 rounded-md border border-gray-300 dark:border-gray-600 form-input" />
      </div>
      <div>
        <label for="description" class="block mb-2 font-semibold text-primaryLight dark:text-primaryDark">Deskripsi</label>
        <textarea id="description" name="description" rows="4" placeholder="Deskripsi singkat kategori" class="w-full p-2 rounded-md border border-gray-300 dark:border-gray-600 form-input resize-y"><?= e($description) ?></textarea>
      </div>
      <div class="flex justify-end gap-4 pt-4">
        <a href="admin_kategori_list.php" class="px-6 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition">Batal</a>
        <button type="submit" class="btn-submit"><?= $is_edit ? "Update Kategori" : "Tambah Kategori" ?></button>
      </div>
    </form>
  </main>
