...
if (isset($_GET['hapus']) && is_numeric($_GET['hapus'])) {
    ...
    $stmt_delete = $conn->prepare("DELETE FROM kategori WHERE id = ?");
    $stmt_delete->bind_param('i', $id_hapus);
    $stmt_delete->execute();
    $stmt_delete->close();
    header("Location: admin_kategori_list.php?status=sukses_hapus");
    exit;
}

$sql = "SELECT id, name, description FROM kategori ORDER BY name ASC";
...
<head>
  ...
  <style>
    ...
    .btn-primary { background-color: var(--primary); color: white; padding: 0.5rem 1.5rem; border-radius: 0.75rem; font-weight: 600; text-decoration: none; display: inline-flex; align-items: center; gap: 0.5rem; transition: all 0.3s ease; }
...
<body class="bg-backgroundLight dark:bg-backgroundDark text-textLight dark:text-textDark transition-colors duration-300 min-h-screen flex flex-col">
  ...
  <main class="container mx-auto px-6 py-12 flex-grow">
    <div class="flex justify-between items-center mb-8">
      <h1 class="orbitron text-4xl select-none">Kelola Kategori</h1>
      <a href="admin_kategori_form.php" class="btn-primary">Tambah Kategori</a>
    </div>
        ...
        <tbody>
          <?php if ($result && $result->num_rows > 0): while ($row = $result->fetch_assoc()): ?>
            <tr class="border-b border-gray-200 dark:border-gray-700 hover:bg-blue-100/50 dark:hover:bg-blue-900/20">
              ...
              <td class="px-4 py-2 font-semibold"><?= e($row['name']) ?></td>
              <td class="px-4 py-2 max-w-md truncate"><?= e($row['description']) ?></td>
              <td class="px-4 py-2 text-center">
                <?php
                  $stmt_jumlah = $conn->prepare("SELECT GetGameCountByKategoriId(?) AS jumlah");
                  $stmt_jumlah->bind_param('i', $row['id']);
                  $stmt_jumlah->execute();
                  $result_jumlah = $stmt_jumlah->get_result();
                  $jumlah = 0;
                  if ($result_jumlah && $result_jumlah->num_rows > 0) {
                    $jumlah_row = $result_jumlah->fetch_assoc();
                    $jumlah = $jumlah_row['jumlah'];
                  }
                  $stmt_jumlah->close();
                ?>
                <span class="inline-block px-3 py-1 rounded-full bg-primaryLight/20 dark:bg-primaryDark/30 text-primaryLight dark:text-primaryDark font-semibold text-sm mb-1">
                  <?= $jumlah ?> Game
                </span><br>
                <a href="admin_kategori_form.php?id=<?= e($row['id']) ?>" class="text-blue-500 hover:underline mr-4">Edit</a>
                <a href="admin_kategori_list.php?hapus=<?= e($row['id']) ?>" class="text-red-500 hover:underline" onclick="return confirm('Yakin ingin menghapus kategori: <?= addslashes(e($row['name'])) ?>?')">Hapus</a>
              </td>
            </tr>
          <?php endwhile; else: ?>
            <tr><td colspan="4" class="text-center py-8">Belum ada kategori.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
